...
| `data` | object[] | Yes | Dados agregados |
...
| `groupBy` | enum: category, contact, tag | Yes | Campo para agrupamento dos dados |
| `sortOrder` | enum: asc, desc | No | Ordem de classificação |
| `aggregationDirection` | enum: IN, OUT | Yes | Direção do lançamento financeiro |

#### `data.data`

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `label` | string | Yes | Rótulo do item agrupado |
| `totalAmount` | string | Yes | Valor total do item agrupado em centavos, em formato string |
