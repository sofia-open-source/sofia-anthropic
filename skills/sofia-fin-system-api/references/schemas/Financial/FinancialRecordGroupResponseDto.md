# FinancialRecordGroupResponseDto
...
|-------|------|----------|-------------|
| `id` | string | Yes | Identificador do grupo. |
| `ownerOrganization` | string | Yes | Identificador da organização dona do grupo. |
| `type` | enum: INTERNAL_TRANSFER, OTHER | Yes | Tipo do grupo. |
| `description` | string | Yes | Descrição do grupo. |
| `financialRecords` | string[] | Yes | Lista de identificadores dos lançamentos financeiros. |
| `populatedFinancialRecords` | object[] | No |  |
| `createdAt` | string | Yes | Creation date of the group. |
| `updatedAt` | string | Yes | Last update date of the group. |
| `deletedAt` | string | No | Deletion date of the group. |
