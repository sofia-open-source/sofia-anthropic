...
| `groupingDate` | enum: dueDate, cashDate, competenceDate | No | Date field used to group records by month. |

#### `data.data`

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `items` | object[] | Yes | Dados mensais dos últimos 12 meses |
| `periodResult` | any | No | Resultado do período (receitas - despesas) em formato string |
